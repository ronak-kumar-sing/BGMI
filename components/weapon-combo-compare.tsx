  ...
  if (combos.length === 0) {
    return (
      <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-lg p-8 text-center">
        <h3 className="text-lg font-medium mb-2">Select Weapon Combos to Compare</h3>
        <p className="text-zinc-400 mb-6">Choose up to 2 weapon combinations to see a detailed comparison.</p>

        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[400px] overflow-y-auto p-2">
          {allCombos.map((combo) => (
            <Button
              key={combo.id}
              variant="outline"
              className="justify-start border-zinc-700 hover:bg-zinc-800 hover:text-white"
    ...
    return (
      <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-lg p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Button variant="outline" size="sm" onClick={() => onSelectCombo(combos[0].id)} className="border-zinc-700">
            Change
          </Button>
        </div>

        <div className="text-center mb-6">
          <p className="text-zinc-400">Select one more weapon combo to compare</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[300px] overflow-y-auto p-2">
          {allCombos
    ...
    <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-lg p-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {combos.map((combo, index) => (
          <div key={combo.id} className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-amber-500">
                {combo.weapons[0]} + {combo.weapons[1]}
              </h3>
              <Button variant="outline" size="sm" onClick={() => onSelectCombo(combo.id)} className="border-zinc-700">
                Change
              </Button>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {combo.idealMaps.map((map) => (
                <Badge key={map} variant="outline" className="bg-zinc-900/50 text-zinc-300 border-zinc-700">
                  {map}
                </Badge>
              ))}

              {combo.playstyles.map((style) => (
                <Badge key={style} variant="outline" className="bg-zinc-900/50 text-zinc-300 border-zinc-700">
                  {style}
                </Badge>
              ))}
            </div>

            <p className="text-sm text-zinc-400 mb-4">{combo.description}</p>

            <div className="flex gap-1">
              {combo.effectiveRanges.map((range) => (
                <Badge key={range} className="bg-amber-500/20 text-amber-500 border-amber-500/30">
                  {range}
                </Badge>
              ))}
            </div>
      ...
      <div className="space-y-4">
        ...
        <div className="border border-zinc-800 rounded-lg overflow-hidden">
          <button
            className="w-full flex items-center justify-between p-4 bg-zinc-900/70 text-left"
          ...
          {expandedSection === "damage" && (
            <div className="p-4 bg-zinc-900/30">
              <div className="grid grid-cols-4 gap-4 mb-4">
                <div></div>
                <div className="text-center text-sm font-medium text-zinc-300">Close Range</div>
                <div className="text-center text-sm font-medium text-zinc-300">Mid Range</div>
                <div className="text-center text-sm font-medium text-zinc-300">Long Range</div>
              </div>

              {combos.map((combo, index) => (
                <div key={combo.id} className="grid grid-cols-4 gap-4 mb-4">
        ...
        <div className="border border-zinc-800 rounded-lg overflow-hidden">
          ...
          >
            <h3 className="font-medium">Recoil Patterns</h3>
          ...
          {expandedSection === "recoil" && (
            <div className="p-4 bg-zinc-900/30">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {combos.map((combo, index) => (
                  <div key={combo.id} className="bg-zinc-900/70 p-4 rounded border border-zinc-800">
                    <h4 className="text-sm font-medium text-amber-500 mb-3">
                      {combo.weapons[0]} + {combo.weapons[1]}
                    </h4>
        ...
        <div className="border border-zinc-800 rounded-lg overflow-hidden">
          ...
          >
            <h3 className="font-medium">Recommended Attachments</h3>
                ...
                {combos.map((combo, index) => (
                  <div key={combo.id}>
                    ...
                    <div className="space-y-3">
                      <div className="bg-zinc-900/70 p-3 rounded border border-zinc-800">
                        <div className="font-medium">{combo.weapons[0]}</div>
                        <div className="text-sm text-zinc-400">{combo.attachments[0]}</div>
                        <div className="text-xs text-zinc-500 mt-1">Ammo: {combo.ammoTypes[0]}</div>
                      </div>

                      <div className="bg-zinc-900/70 p-3 rounded border border-zinc-800">
                        <div className="font-medium">{combo.weapons[1]}</div>
                        <div className="text-sm text-zinc-400">{combo.attachments[1]}</div>
                        <div className="text-xs text-zinc-500 mt-1">Ammo: {combo.ammoTypes[1]}</div>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="border border-zinc-800 rounded-lg overflow-hidden">
          ...
          >
            <h3 className="font-medium">Situational Effectiveness</h3>
          ...
          {expandedSection === "situations" && (
            <div className="p-4 bg-zinc-900/30">
              ...
              {["Urban Combat", "Open Fields", "Building Clearing", "Vehicle Combat", "Final Circle"].map(
                (situation) => (
                  <div key={situation} className="grid grid-cols-3 gap-4 mb-4">
